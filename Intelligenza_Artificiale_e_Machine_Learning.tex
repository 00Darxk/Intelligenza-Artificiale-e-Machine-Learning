...
\hypersetup{
    colorlinks=true,
    linkcolor=black,
    pdftitle={Appunti di Intelligenza Artificiale e Machine Learning},
...
\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.7]{probabilità_temperatura.pdf}%
    \caption{Probabilità Con $\Delta E$ Costante}    
\end{figure}
...
\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.7]{probabilità_valutazione.pdf}%
    \caption{Probabilità Con $T$ Costante}    
\end{figure}

Bisogna trovare una ``link function'' tra l'intervallo $\Delta E/T$ e la probabilità $p$, quindi una funzione che effettua un mapping tra l'intervallo $(-\infty,+\infty)$ della differenza delle valutazioni all'intervallo di probabilità di accettazione $[0,1]$. La funzione utilizzata in questo algoritmo ha l'andamento rappresentato nel grafico precedente. 

La caratteristica di poter scegliere come passo uno stato peggiore rende possibile a questo algoritmo di evitare stati peggiori.  
...
dell'algoritmo. Il valore di $T$ parte da un valore elevato, per poi diminuire nel tempo, come se fosse la temperatura durante un processo di tempratura, per cui l'algoritmo si 
...
\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.7]{link_function_simulated_annealing.pdf}%
    \caption{Diverse Link-Function per $T$ Costante}%
\end{figure}
...
altri operatori \verb|is| ed \verb|is not|. Inoltre sono presenti due versione degli operatori logici \verb|&&| e \verb|and|, \texttt{||} e \verb|or| e \verb|!=| e \verb|not|. 
...
Per importare moduli contenenti altre funzioni si utilizza \verb|import| seguito dal nome del modulo da scaricare, che crea un object module con quel nome, si può 
...
La classe \verb|Node| è sempre la stessa, viene definita con un attributo \verb|.size| 
...
\begin{equation}
    \text{RMSE}=\displaystyle\sqrt{\displaystyle\frac{1}{N}\sum_{i=1}^N\left(\texttt{pred}_i-\texttt{true}_i\right)^2}
\end{equation}

\subsection{Valutazione delle Prestazioni: Regressione e Classificazione}
...
delle metriche per poter valutare questi diversi modelli.

\subsubsection{Training, Generalization e Test Error}

Nel caso della regressione si definisce quindi una 
funzione di ``Loss'', utilizzata anche in altri modelli, che rappresenta quanto si perde utilizzando la previsione del modello $f_{\vect{\hat{w}}}(\vect{x})$, dopo essere stato addestrato, invece che il valore reale $\hat{y}$:
\begin{equation}
...
basa sul concetto di evoluzione, dove la popolazione più forte che sopravvive consiste nella combinazione migliore di iperparametri. Una sottoclasse consiste negli algoritmi genetici, che verranno trattati approfonditamente in successivi corsi di Intelligenza Artificiale. 
...
I gradi di libertà sono le variabili da specificare affinché sia individuato in maniera univoca il modello utilizzato. Per le reti dipendono dal numero dei neuroni, numero connessioni, etc. Di solito il numero dei gradi di libertà dipende da un iperparametro. 
...
La matrice $n\times n$, presente sulle righe i valori reali e sulle colonne i valori predetti. 
Un classificatore sempre accurato avrebbe valori non nulli presenti solamente sulle diagonali, quindi avrebbe un'esatta corrispondenza tra i valori predetti e quelli 
...
\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.7]{matrice_confusione.pdf}%
...
Sulla base della curva ROC per valutare se un classificatore è meglio di un altro si determina il suo integrale sotto la curva (AUC). Questo è uno scalare nell'intervallo $[0,1]$:
\begin{equation*}
    \text{AUC}=\displaystyle\int_0^1\text{ROC}(\text{FPR})\text{dFPR}
\end{equation*} 
Questo determina la prestazione media del classificatore, può essere calcolato come un 
integrale numerico. Un classificatore casuale ha una percentuale di falsi positivi pari al 
50\%, come la percentuale di falsi negativi, quindi la curva ROC è una bisettrice del piano, poiché all'aumentare della soglia le due grandezze TPR e FPR aumentano analogamente 
...
\begin{equation}
    \text{s}=\displaystyle\frac{\text{TN}}{\text{TN}+\text{FP}}=\text{TNR}
\end{equation}
...
viene classificato. Per cui le valutazioni devono essere divise in queste due fasi, prima si controlla che individua correttamente un oggetto tramite la sua bounding box, ed in seguito determinare che la classificazione sia corretta. 
...
classe errata o bounding box non trovata, o sbagliata. La prima fase coincide con un modello di regressione e può essere analizzata con gli strumenti descritti nelle sezioni precedenti, per risolvere il problema di segmentazione o identificazione della bounding box, la seconda fase consiste nel classificare gli oggetti contenuti nelle bounding box o segmenti identificati. 
\begin{itemize}
...
insieme $C$ convesso, ed una retta:
\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.7]{insieme_convesso.pdf}%
    \caption{Insieme Convesso $C$ Bidimensionale}%
\end{figure}
...
\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.7]{gradiente_monodimensionale.pdf}%
...
della funzione:
\begin{equation*}
    \forall\vect{v},\vect{w}\in \mathbb{D}\implies g(\vect{w})\geq g(\vect{w})+\nabla g(\vect{v})^T(\vect{w-\vect{v}})
\end{equation*}

\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.7]{proprietà_gradiente.pdf}%
\end{figure}
...
variabili $w_0$ e $w_1$:
\begin{equation}
    \displaystyle\frac{\partial g(\vect{w})}{\partial w_0}
\end{equation}
...
\begin{gather*}
    \lim_{\Delta w_1\to 0}\displaystyle\frac{g(\overline{w}_1,\overline{w}_1+\Delta w_1)-g(\overline{w}_0,\overline{w}_1)}{\Delta w_1}=\frac{\partial g(\vect{w})}{\partial w_1}
\end{gather*}
...
\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.8]{gradiente.pdf}%
    \caption{Gradiente}
\end{figure}
...
\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.8]{derivata_direzionale.pdf}%
    \caption{Derivata Direzionale}
\end{figure}
...
\begin{minted}[escapeinside=//, mathescape]{c}
    /$\vect{w}^{(1)}$/ <- /$\vect{0}$/
    t <- 1
    while /$||\nabla g(\vect{w}^{(t)})||_2>\varepsilon$/
        /$\vect{w}^{(t+1)}\rightarrow\vect{w}^{(t)}+\alpha\nabla g(\vect{w}^{(t)})$/
        t <- t + 1
\end{minted}

\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.8]{gradient_descent.pdf}%
...
\begin{gather*}
    \begin{matrix}
        ...
        \end{bmatrix}&
        \vect{\phi}(\vect{x}_i)=\begin{bmatrix}
...
\begin{equation*}
    y_i=\vect{w}^T\cdot\vect{\phi}(\vect{x}_i)+\varepsilon_i=\vect{\phi}(\vect{x_i})^T\cdot\vect{w}+\varepsilon_i
\end{equation*}
...
\begin{gather*}
    ...
    \end{bmatrix}\cdot\begin{bmatrix}
        \strut w_{0}\\\vdots\\\strut w_{D}
    \end{bmatrix}+\begin{bmatrix}
        \strut \varepsilon_{1}\\\vdots\\\strut \varepsilon_{N}
    \end{bmatrix}\\
...
Il processo di addestramento di un modello di regressione può essere schematizzato nel seguente modo:
\begin{figure}[H]%
    \centering%
    \includegraphics{processo_training.pdf}%
    \caption{Processo di Training}%
\end{figure}
...
Una soluzione consiste nell'utilizzare tre diversi set, se ciò è permesso dal numero di osservazioni, creando un ulteriore validation set. Se si risparmiano osservazioni sul valid set, allora non si crea un modello adeguato, se si risparmia sul test set allora non viene valutato adeguatamente il modello. 
...
Dato uno spazio di probabilità $(\Omega, \mathscr{A},P)$due eventi $A, B\in\mathscr{A}$ si definiscono indipendenti se 
...
Si suppone di avere uno spazio $\vect{V}$ $d$-dimensionale di pattern e un insieme di $s$ classi $W=\left\{w_1\cdots,w_s\right\}$ disgiunte costituite da elementi di $\vect{V}$. 
...
La probabilità di errore corrispondente si ottiene dalla probabilità che un pattern $\vect{x}\in\vect{V}$, di classe $w_j$, venga identificato come appartenente alla classe $w_i$, nel dominio di definizione $R_i$. 

\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.7]{errore.pdf}%
    \caption{Errore di Classificazione Binaria}
\end{figure}

Questo si effettua per entrambi le classi e si somma la probabilità di errore in entrambi i casi:
\begin{equation*}
...
\begin{equation}
    P(w_i)=\displaystyle\frac{\text{dim}\,\Omega_i}{\text{dim}\,\Omega}
\end{equation}
Per calcolare la densità probabilità condizionale $p(\vect{x}|w_i)$ il procedimento è più complesso. Dato un pattern $\vect{x}\in\vect{V}$, si può considerare un cerchio di raggio $r$, dove $r$ rappresenta un iperparametro, e si può stimare la densità di probabilità contando le occorrenze delle varie classi $w_j\in W$ che cadono nell'intorno centrato in $\vect{x}$ $I_r(\vect{x})$:
\begin{equation}
    P(\vect{x}|w_i)=\displaystyle\frac{\text{dim}\,I_r^i(\vect{x})}{\text{dim}\,\Omega}
\end{equation}

\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.7]{massima_verosimiglianza.pdf}%
...
\begin{figure}[H]%
    \centering%
    \includegraphics[scale=0.7]{gaussiana.pdf}%
    \caption{Gaussiana}%
\end{figure}
...
In generale la dimensione $d$ della gaussiane dipende dal numero delle features, ed il numero delle gaussiane dal numero di classi. 
...
\begin{figure}[H]%
    \centering
    \savebox{\tempbox}{\hbox{\includegraphics[scale=0.5]{covarianza_positiva.pdf}}}
    \subfloat[\centering $\sigma_{ij}=\sigma_{ji}=0$]{\raisebox{\dimexpr.5\ht\tempbox-.5\height\relax}{\includegraphics[scale=0.5]{covarianza_nulla.pdf}}}%
    \qquad%
    \subfloat[\centering $\sigma_{ij}=\sigma_{ji}>0$]{\usebox{\tempbox}}%
    \qquad%
    \subfloat[\centering $\sigma_{ij}=\sigma_{ji}<0$]{\raisebox{\dimexpr.5\ht\tempbox-.5\height\relax}{\includegraphics[scale=0.5]{covarianza_negativa.pdf}}}%
...
\begin{equation}
    ...
    \end{bmatrix}=\displaystyle\frac{1}{N}\sum_{i=1}^N\vect{x}_i
\end{equation}
...
\begin{equation}
    \Sigma=\displaystyle\frac{1}{N}\sum_{i=1}^N(\vect{x}_i-\vect{\mu})\cdot(\vect{x}_i-\vect{\mu})^T
\end{equation}

\begin{figure}[H]%
    \centering 
    \savebox{\tempbox}{\hbox{\includegraphics[scale=0.8]{distribuzione_multinormale_curve_livello.pdf}}}
    \subfloat[\centering Vista Laterale]{\raisebox{\dimexpr.5\ht\tempbox-.5\height\relax}{\includegraphics[scale=0.8]{distribuzione_multinormale.pdf}}}%
    \qquad
    \subfloat[\centering Vista Verticale]{\usebox{\tempbox}}%
...
\begin{figure}[H]%
    \centering
    \savebox{\tempbox}{\hbox{\includegraphics[scale=0.8]{superficie_separazione_curve_livello.pdf}}}
    \subfloat[\centering Vista Laterale]{\raisebox{\dimexpr.5\ht\tempbox-.5\height\relax}{\includegraphics[scale=0.8]{superficie_separazione.pdf}}}%
    \qquad
    \subfloat[\centering Vista Verticale]{\usebox{\tempbox}}%
    \caption{Divisione Superficie Decisionale fra Due Distribuzioni Normali Bivariate}
\end{figure}
...
Nel momento in cui bisogna solamente assegnare una classe, non è necessario conoscere l'intervallo di confidenza, e quindi è sufficiente calcolare il numeratore del teorema di Bayes, poiché il denominatore è sempre uguale alla probabilità assoluta. La classe di un certo pattern $\vect{x}$ è quindi dato dal prodotto massimo tra la probabilità condizionale rispetto ad una classe $w_j$ e la probabilità a priori della suddetta classe:
\begin{equation}
    b=\text{argmax}\left\{p(\vect{x}|w_j)\cdot P(w_j)\right\}
\end{equation}
Se le matrici di covarianza sono tutte uguali, non è necessario calcolare la probabilità assoluta per ogni classe, dato che il denominatore è uguale, bisogna solo calcolare l'esponente dell'esponenziale. Solamente il prodotto tra $(\vect{x}-\vect{\mu})^T\cdot(\vect{x}-\vect{\mu})$, poiché $\Sigma$ è uguale, e si assegna la classe per cui questo valore è minimo. La distanza di Mahalanobis in questi casi diventa semplicemente una distanza euclidea. 
...
\subsubsection{Approccio Non Parametrico}

Quando si hanno pochi campioni conviene scegliere l'approccio parametrico, dove si ipotizza di conoscere la distribuzione dei campioni, ma se i campioni sono molti allora si può stimare questa funzione direttamente. 
Con pochi campioni si preferisce una soluzione più semplice per diminuire l'overfitting, poiché una gaussiana ha due gradi di libertà il valore medio o l'eventuale vettore medio, e la varianza o matrice di covarianza, infatti il modello potrebbe essere troppo complesso sui dati di training. 

L'approccio parametrico può portare a dei problemi, poiché in base alla situazione, quest'ipotesi forte potrebbe non coincidere alla realtà. Ovvero la distribuzione dei campioni non rappresenta una gaussiana. Si possono effettuare diversi test formali ed empirici per determinare se veramente si tratta di una gaussiana per determinare se l'approccio sia corretto. 
In caso è corretto si calcolano i parametri statistici tramite il metodo della verosimiglianza. 
...
Poiché all'aumento delle dimensioni, il volume dello spazio aumenta considerevolmente da diminuire notevolmente la precisione del modello. I campioni diventano sparsi all'aumentare delle dimensioni dell'ipercubo dello spazio dei campioni. 
...
\begin{equation*}
    P(k\text{ su }n)=\begin{pmatrix}
        n\\k
    \end{pmatrix}p^k\cdot(1-p)^{n-k}
\end{equation*}

Formalizzando il problema in questo modo si può determinare la probabilità che un campione $\vect{x}$ cada in una regione $\mathbb{R}$ oppure no.
\begin{equation*}
    P_1=\displaystyle\int_{\mathbb{R}}p(\overline{\vect{x}})\text{d}\overline{\vect{x}}
\end{equation*}
Si può quindi estendere la probabilità che tra $n$ pattern indipendenti $k$ cadano all'interno di $\mathbb{R}$:
\begin{equation*}
    P(k\text{ su }n)=\begin{pmatrix}
        n\\k
    \end{pmatrix}P_1^k\cdot(1-P_1)^{n-k}
\end{equation*}
...
\begin{gather*}
    P_1=\displaystyle\int_{\mathbb{R}}p(\overline{\vect{x}})\text{d}\overline{\vect{x}}\approx p(\vect{x})\cdot V\\
    p(\vect{x})=\displaystyle\frac{P_1}{V}=\frac{k}{n\cdot V}\tageq
\end{gather*}

Questo volume nel caso più semplice è un ipercubo $d$-dimensionale definito dalla funzione $\varphi$:
\begin{equation}
    \varphi(\vect{u})=
    \begin{cases}
        1&|u_j|\leq\displaystyle\frac{\strut 1}{\strut 2},\,j=1,\cdots,d\\
        0&\text{altrimenti}
    \end{cases}
\end{equation}
...
\begin{equation*}
    k_n=\displaystyle\sum_{i=1}^n\varphi\left(\frac{\vect{x}_i-\vect{x}}{h_n}\right)
\end{equation*}

Sostituendo $k_n$ nella formula ottenuta precedente dal processo di Bernoulli: 
\begin{equation}
    p(\vect{x})=\displaystyle\frac{1}{n\cdot V}\displaystyle\sum_{i=1}^n\varphi\left(\frac{\vect{x}_i-\vect{x}}{h_n}\right)
\end{equation}


L'ampiezza è un iperparametro, e più la finestra è piccola, più la stima è statisticamente instabile e rumorosa, ovvero la differenza tra due classi è molto piccola. 
Se la finestra è grande invece la stima è più stabile, ma sfocata. 
...
\begin{equation}
    \varphi(\vect{u})=\displaystyle\frac{1}{(2\pi)^{d/2}}e^{-\frac{\vect{u}^T\vect{u}}{2}}
\end{equation}

Si prende quindi come iperparametro la deviazione standard della gaussiana per definire la dimensione di questa finestra, invece del lato dell'ipercubo. 
Questa rappresenta una funzione Soft Kernel, dove il valore dell'iperparametro $h$ non è legato alla lunghezza del lato ma all'ampiezza della funzione scelta per sostituirlo. 
